<template>
    <div class="h-full py-8 px-6 space-y-6 rounded-xl border border-gray-200 bg-white p-20">
        <svg class="w-40 m-auto opacity-75" viewBox="0 0 146 146" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <defs>
                ...
                <linearGradient id="paint1_linear_622:13617" x1="1.74103e-06" y1="8.70228e-06" x2="6.34739e-08" y2="140.677"
                    gradientUnits="userSpaceOnUse">
                ...
                <linearGradient id="paint3_linear_622:13617" x1="1.74103e-06" y1="8.70228e-06" x2="6.34739e-08" y2="140.677"
                    gradientUnits="userSpaceOnUse">
                    <stop stop-color="#4DFFDF" />
                    <stop offset="1" stop-color="#4DA1FF" />
                </linearGradient>
        ...
        <div>
            <h5 class="text-xl text-gray-600 text-center">Order Status</h5>

        </div>
        <table class="w-full text-gray-600">
            <tbody>
                <tr>
                    <td class="py-2">Completed Orders</td>
                    <td class="text-gray-500">{{ completedOrders }}</td>
                    <td>
                ...
                <tr>
                    <td class="py-2">Pending Orders</td>
                    <td class="text-gray-500">{{ pendingOrders }}</td>
                    <td>
                        <svg class="w-16 ml-auto" viewBox="0 0 68 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect opacity="0.4" width="17" height="21" rx="1" fill="#e4e4f2" />
                            <rect opacity="0.4" x="19" width="14" height="21" rx="1" fill="#e4e4f2" />
                ...
                <tr>
                    <td class="py-2">Out for Delivery</td>
                    <td class="text-gray-500">{{ outForDeliveryOrders }}</td>
                    <td>
                        <svg class="w-16 ml-auto" viewBox="0 0 68 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect opacity="0.4" width="17" height="21" rx="1" fill="#e4e4f2" />
                            <rect opacity="0.4" x="19" width="14" height="21" rx="1" fill="#e4e4f2" />
                ...
                <tr>
                    ...
                    <td class="text-gray-500">{{ cancelledOrders }}</td>
                    <td>
                        <svg class="w-16 ml-auto" viewBox="0 0 68 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect opacity="0.4" width="17" height="21" rx="1" fill="#e4e4f2" />
                            <rect opacity="0.4" x="19" width="14" height="21" rx="1" fill="#e4e4f2" />
    ...
    watch: {
        orders: {
            handler: function (newOrders) {
                this.completedOrders = newOrders.filter(order => order.status === 'completed').length;
                this.pendingOrders = newOrders.filter(order => order.status === 'pending').length;
                this.outForDeliveryOrders = newOrders.filter(order => order.status === 'delivery').length;
                this.cancelledOrders = newOrders.filter(order => order.status === 'failed').length;
            },
            immediate: true
        }
    }
