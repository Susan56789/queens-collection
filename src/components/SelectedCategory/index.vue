...
export default {
    ...
    methods: {
        ...
        async addToCart() {
            try {
                ...
                const product = await this.selectedProduct;
        ...
        async fetchProductsByCategory(categoryName) {
            try {
                const response = await axios.get(`http://localhost:3000/api/category/name?CategoryName=${categoryName}`);
    ...
    watch: {
        '$route.params.name'(newCategoryName) {
            ...
            this.fetchProductsByCategory(newCategoryName);
        },
    },
    mounted() {
        this.fetchData();
        this.selectedCategoryName = this.$route.params.name;
        if (this.selectedCategoryName) {
            this.fetchProductsByCategory(this.selectedCategoryName);
        }
    },
};
</script>
